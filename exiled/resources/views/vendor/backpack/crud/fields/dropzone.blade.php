<div class="form-group col-md-12">
	...
	<div class="dropzone sortable dz-clickable sortable">
	    <!-- <div class="message">
			Soltar aquí los archivos o haga clic para cargar.
	    </div> -->
		@if ($entry->{$field['name']})
		    @foreach($entry->{$field['name']} as $key => $image)
				@if($image)
			    <div class="dz-preview" data-id="{{ $key }}" data-path="{{ $image }}">
					<a target="_blank" href="{{ asset('uploads/'.$image) }}">
			    		<img class="dropzone-thumbnail" src="{{ asset('uploads/'.$image) }}">
					</a>
					<a class="dz-remove" href="javascript:void(0);" data-remove="{{ $key }}" data-path="{{ $image }}">Eliminar</a>
			    </div>
				@endif
			@endforeach
		@endif
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
  ...
  {{-- push things in the after_scripts section --}}
      ...
      @push('crud_fields_scripts')
		...
		<link rel="stylesheet" href="{{ asset('/css/dropzone.css') }}"></script>
			...
			var dropzone = new Dropzone(".dropzone", {
			    ...
			    success : function(file, status) {
			        ...
			        $.each(status.images, function(key, image_path) {
		        	 	$('.dropzone').append('<div class="dz-preview" data-id="'+key+'" data-path="'+image_path+'"><img class="dropzone-thumbnail" src="{{ url('uploads') }}/'+image_path+'" /><a class="dz-remove" href="javascript:void(0);" data-remove="'+key+'" data-path="'+image_path+'">Remove file</a></div>');
			        });
					...
					if (status.success) {
						...
						alert('¡Tipo de fichero no permitido!, debe ser jpg, gif o png.');
					}
			    ...
			    stop: function (event, ui) {
			        ...
			        .done(function(status) {
						...
						if (status.success) {
							...
							notification_type = 'error';
							alert('Ocurrió un error al ordenar las imágenes');
						}
			...
			$(document).on('click', '.dz-remove', function () {
				...
				.done(function(status) {
					...
					if (status.success) {
						...
						notification_type = 'error';
						alert('Ocurrió un error al eliminar la imágen.');
					}
